...
### Added
- 2026-06-09 [P2.0–P2.6 实现完成 + Claude 逐单审查 → 签发 P2-review-fixes] OpenClaw 已实现 P2.0b/P2.1–P2.6 于 `feature/p2.0-foundation`（领先 main 88 commit；最近 3 commit 仍在修编译错误）。Claude 静态逐单审查（对照 ARCHITECTURE 附录 B 门禁）：**亮点**——P2.0b 存档迁移真接进读档路径（`FileSaveRepository.Load` JObject→`SaveMigrationRunner.Upgrade`→ToObject、链式 0→1→2、纯整数确定性、边界有防护）；P2.5 空间索引 C-5 真偿还（`WorldState.provinceUnitIds` + `ReadModelBuilder` 真改用 `GetUnitsInProvince`、移动/造兵/战斗/消灭路径均同步、无幽灵驻军）；P2.1 D3「绝不卖战力」守住（旧抽卡/买券命令枚举已删、`GachaResolver`/`ShopResolver` 标 Obsolete 挖空、确定性 id 保留未回退 Guid）。分层（Presentation 零 Domain/Sim、核心层零 UnityEngine）+ 确定性大体守住。**发现问题 → 签发 `WorkOrders/P2-review-fixes.md`**：🔴 F1 P2.6 遥测自写 `JsonUtility`（违规则 8 + 匿名类型 `GetFields` 取不到 public property → `turn_advanced` 数据序列化成空）；🟠 F2 `TerrainAggregator.GetBaseDefenseWeight` 重复硬编码一份与 `terrainDefenseMult` 漂移的权重表 + 平票裁决依赖 Dictionary 遍历序（确定性隐患）；🟠 F3 `AdjacencyResolver.MatchesHandwritten` 误导性死方法（永远 return true）；🟡 F4 P2.1 收尾（`CardDrawnEvent` 命名残留 + 商城买券钩子未清 + 废弃 gacha 随机参数死配置）；🟡 F5 `SpatialIndexTests` 一致性测试自手动同步索引（验不了产品路径）；🟡 F6 仓库卫生（26 个 .meta + Tools/ 未提交）。**既有债**（非 P2 引入·T5/B1.5 遗留）F7：`EconomyResolver` `stabilityMod`/`inflationPenalty` float 参与税收计算（确定性破口，C12 整数化漏网）→ 建议独立专项 + 需人类确认数值等价（规则 9/14）。**关键闸门**：全程无 P2 运行证据，合 main 前须补 batchmode 编译 + EditMode 全绿 artifact（"只信代码 + 要证据"）。涉规则 2/4/5/6/7/8。
- 2026-06-08 [P3 前置基础设施签发 P3a · 其余 P3/P4 暂不签] 人类要求继续签 P3/P4,Claude **push back**:P3(服务端/账号/PvP 撮合/变现实装/LiveOps)仅 ROADMAP 占位、零技术设计,且形态**取决于 P2.6 真人验证**;P4 从未规划。硬签 = 逼 OpenClaw 做架构决策(违规则 12)+ 烧额度写无方向废码。**只前置唯一合格的独立单**:`WorkOrders/P3a-determinism-replay.md`——确定性回放系统(`ReplayData`/`ReplayRecorder`/`ReplayPlayer` + record→replay `HashWorld` 等价 + **黄金回放回归测试**把"确定性"变成 CI 每次拦的硬约束)。它是异步 PvP/反作弊/观战地基,**独立于地图、不依赖 P2.6 验证、规格明确**,可与 P2 并行。其余 P3/P4 待 **P2.6 验证 + Claude 出设计文档**后再签(YAGNI:先骨架后收窄,不为没验证的方向投规格)。涉规则 9/12/14。
- 2026-06-08 [Phase 2 工作单批量签发 P2.1–P2.6] 应人类需求(OpenClaw 额度到期前压大体量),一次签发 Phase 2 剩余实现单。**执行顺序**:P2.1 独立 / **P2.2→P2.3→P2.4→P2.5 强依赖必须按序** / P2.6 埋点。**签发≠免审**:仍逐单按序交付 + CI + Claude 审(P2.0b 已证"一次写堆不跑测试会埋雷")。单据:`P2.1` 抽卡退役→战功点定向解锁养成(复用 gachaTickets 字段当"战功点"不破存档、移随机抽卡/保底/买券、`UnlockCommander` 定向解锁+升星、横向特化,C-7);`P2.2` 地图三层(`TileState`+tileIds+邻接自动推导+schema bump 2+`Migration_1to2`,6省×4格,C-2 / MAP §2-3);`P2.3` 渲染换栈(世界空间 Tilemap+正交相机、UI 退 HUD,C-8 / MAP §5);`P2.4` 地形玩法(格地形→省主导聚合+补给走格+地形倍率移 config,MAP §4);`P2.5` 中等地图(空间索引 C-5+地图编辑器 C-3+~24 省战役);`P2.6` 埋点雏形(本地 Telemetry,**验证主体留真人+Claude**)。**Claude 代拟数值**(战功点成本/地图 24 省/地形倍率/主导地形聚合)均标人类可调(规则 14)。每单 DoD 含 **CI 必绿 + 本轮 artifact + 真实截图**(Phase1 截图造假已记)。涉规则 7/12/14;债 C-2/C-3/C-5/C-7/C-8。
