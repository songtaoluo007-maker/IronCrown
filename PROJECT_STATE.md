...
> 最后更新:2026-06-10(Phase 2 P2.0–P2.6 已实现于 `feature/p2.0-foundation`·P2-review-fixes F1–F6 已修复并提交 `89e766e`·P3a 确定性回放系统已实现 `a59e3f5`·待 Claude 复审 + 运行证据)。
...
| **P2.1** | 抽卡退役→战功点定向解锁(D3 守住·确定性 id) | ✅ F4 修复完成 |
| **P2.2** | 地图三层数据(TileState+tileIds+邻接自动推导) | ✅ F3 修复完成 |
| **P2.3** | 渲染换栈(世界空间 Tilemap+正交相机) | ✅ 通过 |
| **P2.4** | 地形玩法(格地形→省主导聚合+地形倍率) | ✅ F2 修复完成 |
| **P2.5** | 空间索引(C-5 真偿还)+地图编辑器+~24 省 | ✅ F5 修复完成 |
| **P2.6** | 本地 JSON 埋点雏形 | ✅ F1 修复完成 |
| **P2-review-fixes** | F1–F6 修复 + .meta 卫生 | ✅ 完成(`89e766e`) |
| **P3a** | 确定性回放系统 + 黄金回放回归测试 | ✅ 完成(`a59e3f5`) |
| **P2 合 main 闸门** | Claude 复审 + 运行证据(artifact/截图) | ⏳ 未达 |
| **Phase 3** | 服务端/账号/PvP/LiveOps(P3a 确定性回放可并行) | 🗺️ P3a 完成·P3b+ 待签 |
